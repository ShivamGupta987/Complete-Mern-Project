...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const offset = window.scrollY; // Fix typo: window.scrollY instead of window.scrolly
      if (offset > 0) {
        setSticky(true);
      } else {
        setSticky(false);
    ...
    return () => {
      window.removeEventListener("scroll", handleScroll); // Fix: Remove event listener on cleanup
    };
  }, []);
